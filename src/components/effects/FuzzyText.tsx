...
export const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      } else if (computedFontFamily.includes('var(')) {
        const temp = document.createElement('span');
        temp.style.fontFamily = computedFontFamily;
        document.body.appendChild(temp);
        computedFontFamily = window.getComputedStyle(temp).fontFamily;
        document.body.removeChild(temp);
      }

      let numericFontSize = 64;
      ...
      const fontSizePx = `${numericFontSize * dpr}px`;
      const fontString = `${fontWeight} ${fontSizePx} ${computedFontFamily}`;
      ...
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            isScrollGlitching = true;
            setTimeout(() => {
              isScrollGlitching = false;
            }, 800); // 800ms duration (gives it plenty of frames)
          }
        });
      }, { threshold: 0.5 });
      ...
      const run = (timestamp: number) => {
        ...
        if (isClicking) {
          targetIntensity = 1;
        } else if (isGlitching || isScrollGlitching) {
          targetIntensity = 0.8;
        } else if (isHovering) {
