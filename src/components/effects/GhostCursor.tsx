...
export const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // We attach events to the document so it tracks cursor globally
  ...
  const mergedStyle = useMemo<React.CSSProperties>(() => ({ 
    zIndex, 
    position: 'fixed', // Keep it fixed so it tracks over the whole window
    pointerEvents: 'none', 
    ...style 
  }), [zIndex, style]);

  return (
    <div ref={containerRef} className={`absolute inset-0 ${className ?? ''}`} style={mergedStyle} />
  );
};
